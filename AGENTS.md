...
- **UI tokens**: colors come from `AppColors.*` semantic getters (`brandPrimary`, `statusOnline`, `statusAlert`, `statusNeutral`, `textSecondary`, `surfaceCard`, …); spacing / radius / icon sizes from `AppSpacing` / `AppRadius` / `AppSize` / `AppIconSize` in `lib/shared/theme/app_tokens.dart`. Never write `Color(0x…)` or arbitrary paddings in feature or shared-widget code — `test/theme/no_hardcoded_colors_test.dart` fails on new literals. Full table with usage rules: `docs/design-tokens.md`.
- **User-facing errors**: never render `e.toString()`; pass the error object to `PetToast.error(context, error)` / `AppErrorView(error: …)` or map it with `ErrorPresenter.describe(error)`. Whether raw error text is appended is controlled by `assets/config/app_flags.json` (`showRawError`), not by build mode.
